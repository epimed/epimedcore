...
class BenjaminiHochberg():
    "Benjamini–Hochberg procedure to calculate false discovery rate (FDR)"
    
    def __init__(self) -> None:
        pass
    
    def calculate(self, p_vals: pd.Series) -> pd.Series:
        ranked_p_values = rankdata(p_vals)
        fdr = p_vals * len(p_vals) / ranked_p_values
        fdr[fdr > 1] = 1
        return fdr.loc[p_vals.index]
...
class FisherExact():
    ...
    def calculate(self, table) -> float:
